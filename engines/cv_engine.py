from engines.parser_engine import extract_images_from_pdf
    ...
    for img in images:
        ...
        if w > 0 and h > 0:
            ...
            if aspect > 3 or aspect < 0.33:
                photo_analysis["photo_quality_notes"].append(
                    f"Page {page}: Unusual aspect ratio ({aspect:.1f}) - may be diagram/figure"
                )
            if w < 100 or h < 100:
                photo_analysis["photo_quality_notes"].append(
                    f"Page {page}: Small image ({w}x{h}) - likely icon/logo"
                )
        if img.get("bbox"):
            photo_analysis["photo_quality_notes"].append(
                f"Page {page}: Image has position data - likely embedded photo with layout context"
            )
