...
def create_spatial_map(findings, property_data, user_floorplan=None):
    ...
    return {
        ...
        "floor_plan_note": (
            ...
            if has_user
            else "Template layout derived from beds/baths metadata. Upload a floorplan or "
...
def _match_to_room(location_text, rooms, system):
    ...
    for room in rooms:
        if default.lower() in room.get("name", "").lower():
            return room
    return (
        rooms[0] if rooms else {"name": "Unknown", "x": 5, "y": 5, "width": 3, "height": 2, "floor": "Main"}
    )
