...
def analyze_investor_deal(property_data, findings, cost_matrix, capex_analysis,
    ...
    return {
        ...
        "risk_assessment": {
            "repair_risk": "HIGH" if total_repair > arv * 0.25 else "MEDIUM" if total_repair > arv * 0.15 else "LOW",
            "hold_risk": "HIGH" if holding_period_months > 9 else "MEDIUM" if holding_period_months > 6 else "LOW",
            "capex_risk": "HIGH" if capex_weighted > 10000 else "MEDIUM" if capex_weighted > 5000 else "LOW",
            "overall_deal_quality": _assess_deal_quality(max_offer, arv, total_repair, cap_rate, cash_on_cash),
        },
        "investment_analysis": _generate_investment_analysis(listing_price, arv, max_offer, total_repair, monthly_rental, cap_rate, cash_on_cash, prop_age, sqft),
    }
...
def _generate_investment_analysis(listing_price, arv, max_offer, total_repair, monthly_rental, cap_rate, coc, prop_age, sqft):
    analysis = []
    repair_to_arv = total_repair / max(arv, 1) * 100
    if repair_to_arv > 25:
        analysis.append(f"REPAIR WARNING: Repair costs ({repair_to_arv:.1f}% of ARV) exceed 25% threshold. This significantly increases flip risk.")
    ...
    if sqft < 1200:
        analysis.append("Smaller property may limit rental income potential. Consider target demographic (singles, couples vs families).")
    elif sqft > 3000:
        analysis.append("Larger property may attract higher-quality tenants but also higher maintenance costs.")
    return analysis
            ...
            if asset_type:
                ...
                if dep["failure_probability_24mo"] > 20:
                    year_costs.append({
                        ...
                        "expected_cost": dep["replacement_cost_avg"] * dep["failure_probability_24mo"] / 100,
                    })
