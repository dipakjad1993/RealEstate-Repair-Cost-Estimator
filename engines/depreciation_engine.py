        ...
        for kw in keywords:
            if kw in text_lower:
                ...
                serial_match = re.search(
                    r"serial(?:\s*(?:no|number|#))?\s*[:\s]+\s*([A-Za-z0-9]+)", text, re.IGNORECASE
                )
                if serial_match:
                    asset["serial_number"] = serial_match.group(1).strip()
                year_match = re.search(
                    r"(?:manufactured|mfg|built|installed|year)\s*(?:in\s*)?(\d{4})", text, re.IGNORECASE
                )
                if year_match:
                    asset["year_manufactured"] = int(year_match.group(1))
                age_match = re.search(
                    r"(?:age|approximately|about|~)\s*(\d{1,2})\s*years?", text, re.IGNORECASE
                )
                if age_match:
...
def analyze_all_capex(findings, property_data):
    ...
    for finding in findings:
        ...
        if system in system_estimates:
            ...
            dep = calculate_depreciation_curve(
                sys_info["type"], estimated_age, finding.get("description", "")
            )
    ...
    total_capex_risk_24mo = sum(
        item["replacement_cost_avg"] * (item["failure_probability_24mo"] / 100) for item in capex_items
    )
