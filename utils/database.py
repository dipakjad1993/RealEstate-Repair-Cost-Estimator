...
def save_property(data):
    ...
    """, (
        ...
        data.get('zip_code', ''), data.get('county', ''), data.get('parcel_number', ''),
        data.get('property_type', 'Single Family'), data.get('year_built', 0),
        data.get('square_footage', 0), data.get('lot_size_sqft', 0),
        data.get('bedrooms', 0), data.get('bathrooms', 0), data.get('stories', 1),
        data.get('garage_type', ''), data.get('foundation_type', ''),
        data.get('roof_type', ''), data.get('hvac_type', ''),
        data.get('plumbing_type', ''), data.get('electrical_type', '')
    ))
    conn.commit()
...
def save_inspection_report(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('inspector_name', ''),
        data.get('inspection_date', datetime.now().strftime('%Y-%m-%d')),
        data.get('report_type', 'Full Inspection'),
        data.get('report_filename', ''), data.get('raw_text', ''),
        json.dumps(data.get('parsed_findings', [])), data.get('total_pages', 0),
...
def save_finding(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('report_id'), data.get('property_id'),
        data.get('system_category', ''), data.get('subsystem', ''),
        data.get('component', ''), data.get('location', ''),
        data.get('description', ''), data.get('severity', 'MEDIUM'),
        data.get('severity_score', 3), data.get('estimated_cost_low', 0),
        data.get('estimated_cost_high', 0), data.get('estimated_cost_avg', 0),
        data.get('photo_extracted', 0), data.get('dedup_group', ''),
...
def save_cost_estimate(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('finding_id'), data.get('diy_low', 0), data.get('diy_high', 0),
        data.get('contractor_low', 0), data.get('contractor_high', 0),
        data.get('emergency_low', 0), data.get('emergency_high', 0),
        data.get('local_modifier', 1.0), data.get('zip_code', ''),
        data.get('material_cost_low', 0), data.get('material_cost_high', 0),
        data.get('labor_cost_low', 0), data.get('labor_cost_high', 0),
...
def save_depreciation(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('finding_id'), data.get('asset_type', ''),
        data.get('make', ''), data.get('model', ''),
        data.get('serial_number', ''), data.get('year_manufactured', 0),
        data.get('age_years', 0), data.get('useful_life', 15),
        data.get('remaining_life', 0), data.get('depreciation_pct', 0),
        data.get('replacement_cost_avg', 0), data.get('replacement_cost_low', 0),
        data.get('replacement_cost_high', 0), data.get('failure_probability_24mo', 0),
...
def save_capex_projection(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('finding_id'),
        data.get('asset_type', ''), data.get('current_age', 0),
        data.get('useful_life', 15), data.get('projected_failure_month', 0),
        data.get('projected_failure_year', 0), data.get('replacement_cost_avg', 0),
        data.get('risk_level', 'MEDIUM'), data.get('priority', 3),
        data.get('notes', '')
    ))
    conn.commit()
...
def save_negotiation_strategy(data):
    ...
    """, (
        data.get('property_id'), data.get('finding_id'),
        data.get('strategy_type', ''), data.get('priority_rank', 0),
        data.get('action', ''), data.get('rationale', ''),
...
def save_escrow_holdback(data):
    ...
    """, (
        data.get('property_id'), data.get('finding_id'),
        data.get('defect_description', ''), data.get('contractor_bid', 0),
        data.get('holdback_amount', 0), data.get('holdback_multiplier', 1.5),
        data.get('release_conditions', ''), data.get('milestone_1', ''),
        data.get('milestone_1_pct', 50), data.get('milestone_2', ''),
...
def save_environmental_risk(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('risk_type', ''),
        data.get('risk_level', 'LOW'), data.get('risk_score', 0),
        data.get('description', ''), data.get('insurance_impact', 0),
        data.get('estimated_cost', 0), data.get('mitigation_recommendation', ''),
        data.get('climate_zone', ''), data.get('flood_zone', ''),
        data.get('seismic_zone', ''), data.get('wildfire_risk', '')
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_investor_analysis(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('listing_price', 0),
        data.get('after_repair_value', 0), data.get('max_allowable_offer', 0),
        data.get('total_repair_cost', 0), data.get('holding_costs', 0),
        data.get('closing_costs', 0), data.get('profit_target', 0),
        data.get('cap_rate', 0), data.get('cash_on_cash_return', 0),
        data.get('monthly_rental_est', 0), data.get('noi_annual', 0),
        data.get('capex_reserve_5yr', 0), json.dumps(data.get('depreciation_forecast', []))
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_insurance_analysis(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('finding_id'), data.get('property_id'),
        data.get('red_flag_type', ''), data.get('risk_score', 0),
        data.get('denial_probability', 0), data.get('annual_premium_impact', 0),
        data.get('replacement_cost', 0), data.get('description', ''),
        data.get('recommendation', ''), data.get('insurability_score', 50)
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_market_data(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('zip_code', ''), data.get('state', ''),
        data.get('city', ''), data.get('avg_days_on_market', 30),
        data.get('inventory_count', 0), data.get('active_listings', 0),
        data.get('pending_sales', 0), data.get('median_sale_price', 0),
        data.get('price_per_sqft', 0), data.get('yoy_price_change', 0),
        data.get('months_of_supply', 3), data.get('market_type', 'Balanced'),
...
def save_permit_record(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('permit_type', ''),
        data.get('permit_number', ''), data.get('permit_date', ''),
        data.get('contractor_name', ''), data.get('status', ''),
        data.get('description', ''), data.get('source', 'simulated'),
        data.get('match_status', 'unverified'), data.get('notes', '')
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_legal_addendum(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('addendum_type', 'Repair Request'),
        data.get('addendum_text', ''), json.dumps(data.get('selected_findings', []))
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_sandbox_item(data):
    ...
    """, (
        data.get('property_id'), data.get('finding_id'),
        data.get('selected', 1), data.get('repair_type', 'seller_credit'),
        data.get('override_amount', 0), data.get('notes', '')
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_contractor_bid(data):
    ...
    """, (
        data.get('finding_id'), data.get('property_id'),
        data.get('contractor_name', ''), data.get('contractor_license', ''),
        data.get('contractor_rating', 0), data.get('bid_amount', 0),
        data.get('bid_description', ''), data.get('warranty_terms', ''),
        data.get('timeline_days', 0), data.get('bid_status', 'pending')
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_spatial_data(data):
    ...
    """, (
        data.get('finding_id'), data.get('property_id'),
        data.get('room', ''), data.get('wall', ''),
        data.get('floor_level', 'Main'), data.get('x_position', 0),
        data.get('y_position', 0), data.get('zone', ''),
...
def save_recall_check(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('finding_id'), data.get('manufacturer', ''),
        data.get('model', ''), data.get('serial_number', ''),
        data.get('product_type', ''), data.get('recall_status', 'Not Found'),
        data.get('recall_type', ''), data.get('claim_url', ''),
        data.get('description', ''), data.get('remedy', ''),
...
def save_brokerage_roi(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('broker_id', ''), data.get('agent_id', ''),
        data.get('property_id'), data.get('total_credits_negotiated', 0),
        data.get('total_repairs_requested', 0), data.get('items_requested', 0),
        data.get('items_granted', 0), data.get('negotiation_success_rate', 0),
        data.get('zip_code', ''), data.get('transaction_date', datetime.now().strftime('%Y-%m-%d'))
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_seo_page(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('zip_code', ''), data.get('system_type', ''),
        data.get('avg_cost', 0), data.get('page_slug', ''),
...
def save_lead_activity(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get('visitor_ip', ''), data.get('email', ''),
        data.get('phone', ''), data.get('zip_code', ''),
        data.get('report_filename', ''), data.get('analysis_preview', ''),
        data.get('lead_captured', 0), data.get('source_page', '')
    ))
    conn.commit()
    return cursor.lastrowid
...
def save_audio_transcript(data):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
        data.get('property_id'), data.get('audio_filename', ''),
        data.get('transcript_text', ''), json.dumps(data.get('findings_extracted', [])),
        data.get('duration_seconds', 0), data.get('recorded_by', '')
    ))
    conn.commit()
    return cursor.lastrowid
...
def query_capex_projections(property_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM capex_projections WHERE property_id = ? ORDER BY projected_failure_month ASC", (property_id,))
    results = cursor.fetchall()
    conn.close()
    return [dict(r) for r in results]
...
def query_negotiation_strategies(property_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM negotiation_strategies WHERE property_id = ? ORDER BY priority_rank ASC", (property_id,))
    results = cursor.fetchall()
    conn.close()
    return [dict(r) for r in results]
...
def get_database_stats():
    ...
    stats = {}
    tables = [
        'properties', 'inspection_reports', 'findings', 'cost_estimates',
        'depreciation_data', 'capex_projections', 'market_data',
        'negotiation_strategies', 'environmental_risk', 'insurance_analysis',
        'escrow_holdbacks', 'legal_addenda', 'permit_records',
        'investor_analysis', 'brokerage_roi', 'lead_magnet_activity',
        'seo_pages', 'contractor_bids', 'spatial_data', 'recall_checks',
        'sandbox_items', 'audio_transcripts'
    ]
    for table in tables:
