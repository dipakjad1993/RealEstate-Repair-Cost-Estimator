...
def _classify_severity(text):
    ...
    critical_kw = ["dangerous", "immediate", "safety", "fire", "gas leak", "active leak", "collapse", "hazard", "emergency", "broken", "failing"]
    high_kw = ["damaged", "rust", "corroded", "leaking", "crack", "rot", "water damage", "code violation", "not working", "malfunction"]
    medium_kw = ["worn", "aging", "maintenance", "recommend", "should be", "minor", "slight", "noisy", "staining"]
...
def _classify_system(text):
    text_lower = text.lower()
    system_keywords = {
        "HVAC": ["hvac", "heating", "cooling", "air conditioner", "furnace", "duct", "thermostat", "heat pump"],
...
def _extract_location(text):
    ...
    location_patterns = [
        ...
        "attic", "hallway", "dining room", "utility", "laundry", "master bedroom",
    ]
...
def _extract_subcomponent(text):
    text_lower = text.lower()
    subcomponents = {
        "heat exchanger": "heat exchanger", "blower motor": "blower motor",
        "compressor": "compressor", "capacitor": "capacitor",
        "flashing": "flashing", "shingle": "shingles",
        "gfci": "GFCI outlet", "panel": "electrical panel",
        "pipe": "pipe", "drain": "drain", "faucet": "faucet",
        "foundation": "foundation", "crack": "crack",
        "siding": "siding", "paint": "paint", "deck": "deck",
        "window": "window", "door": "door", "insulation": "insulation",
        "mold": "mold", "moisture": "moisture",
    }
